document.addEventListener('DOMContentLoaded', function () {
  ...
  UIElements.quizForm.addEventListener('submit', showFinalResult);

  UIElements.quizForm.addEventListener('change', (e) => {
    if (e.target.type === 'radio') {
      handleInstantCorrection(e.target);
    }
  });
  ...
  function handleInstantCorrection(selectedInput) {
    ...
    labels.forEach((label) => {
      ...
      } else if (isSelected && radioInput.value !== questionData.answer) {
        label.classList.add('incorrect');
      }
    });
  }

  function showFinalResult(event) {
